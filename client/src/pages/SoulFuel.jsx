...
const SoulFuel = () => {
        ...
        setTimeout(() => {
          setSavedMessages((prev) => {
            const filtered = prev.filter(
              (msg) => msg.message !== data.data.message
            );
  ...
  const saveMessage = () => {
    ...
    const favorites = JSON.parse(
      localStorage.getItem("soulfuelFavorites") || "[]"
    );
    const isAlreadySaved = favorites.some(
      (fav) => fav.message === currentMessage.message
    );

    if (!isAlreadySaved) {
      const updatedFavorites = [
        ...favorites,
        { ...currentMessage, savedAt: new Date() },
      ];
  ...
  return (
    <div
      className={`min-h-screen ${currentTheme.bg} transition-all duration-1000 p-4 md:p-8`}
    >
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div
              className={`relative rounded-3xl shadow-2xl overflow-hidden backdrop-blur-sm transition-all duration-1000 ${
                ...
                {!loading && !error && currentMessage && (
                  <div className="text-center space-y-8">
                    ...
                    <div className="flex justify-center">
                      <FaQuoteLeft
                        className={`text-4xl ${currentTheme.text} opacity-50 transform -translate-x-2`}
                      />
                      <FaQuoteRight
                        className={`text-4xl ${currentTheme.text} opacity-50 transform translate-x-2`}
                      />
                    ...
                    <div className="space-y-6">
                      ...
                      <div
                        className={`border-t ${
                          currentTheme.border
                        } pt-6 transition-all duration-1000 delay-300 ${
                          isAnimating ? "opacity-0" : "opacity-100"
                        }`}
                      >
                        <p
                          className={`text-xl font-medium ${currentTheme.text}`}
                        >
                          — {currentMessage.author}
                        </p>
                        {currentMessage.category && (
                          <span
                            className={`inline-block px-3 py-1 rounded-full text-sm font-medium mt-2 ${currentTheme.bg} ${currentTheme.text} border ${currentTheme.border}`}
                          >
                            {currentMessage.category}
                          </span>
                        )}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      ...
                      onChange={(e) =>
                        updateNotificationSettings("soulfuel", e.target.checked)
                      }
                    />
          ...
          <div className="space-y-8">
                  ...
                  {savedMessages.map((message, index) => (
                    <div
                      ...
                      className={`p-4 rounded-xl border-l-4 ${
                        themes[message.category]?.border ||
                        themes.default.border
                      } bg-white/50 backdrop-blur-sm transition-all duration-300 hover:shadow-md cursor-pointer`}
                      onClick={() => {
            ...
            <div className="bg-gradient-to-br from-purple-500 to-pink-500 rounded-2xl shadow-lg p-6 text-white">
              ...
              <p className="text-sm opacity-90">
                Take a moment to reflect on today's message. Let it inspire your
