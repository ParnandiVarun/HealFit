...
const SMTP_PORT = process.env.SMTP_PORT || 590;
const SMTP_USER = process.env.SMTP_USER;
const SMTP_PASS = process.env.SMTP_PASS;
const FROM_EMAIL =
  process.env.FROM_EMAIL || `"Wellness" <no-reply@example.com>`;

if (!SMTP_HOST || !SMTP_USER || !SMTP_PASS) {
  console.warn(
    "Warning: SMTP not fully configured. forgot-password emails will fail until SMTP vars are set."
  );
}
